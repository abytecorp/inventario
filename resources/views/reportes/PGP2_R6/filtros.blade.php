...
<div class="box box-default">
  <div class="box-body table-responsive no-padding">
    <table class="table table-bordered">
      <tbody>
        <tr class="active">
          <td colspan="3"><strong><i class="fa fa-bars"></i> CONTROL DE CALIDAD Y TRAZABILIDAD</strong></td>
        </tr>
        <tr>
          <td width="20%">
            <!-- ************************************************************************ -->
              ...
              <?php $separador = '';?>
                ...
                @foreach($combos as $rep)
                    @if($separador!=$rep->orden_id)
                      <option value="{{$rep->orden_id}}">Orden de Manufactura: {{$rep->orden_id}} &nbsp;&nbsp;Cliente: {{$rep->cliente}} &nbsp;&nbsp;</option>
...
<script>
  ...
  function actualizar(){
    $("#vistaPrevia").show();
    $("#reporte").load('<?php echo url('/admin/PGP2_R6_REPORTE'); ?>',{
  ...
  function descargar(){
    ...
    location="<?php echo url('/admin/PGP2_R6_REPORTE_DESCARGAR'); ?>?"+parametros;
  }
</script>
@endsection